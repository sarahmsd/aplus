    ...
    <div class="wrapper">
        <div>
            ...
            <table class="fl-table table-style-1">
                ...
                <tbody>
                    @if (!is_null($projets) && is_countable($projets) && count($projets) > 0)
                        @foreach ($projets as $projet)
                        <tr>
                            <td>{{ $projet->nomStartup }}</td>
                            <td>{{ $projet->NomComplet }}</td>
                            <td>{{ $projet->secteur_id == 1 ? 'Technologie' : ($projet->secteur_id == 2 ? 'Industrie' : 'Autre') }}</td>
                            <td>{{ $projet->siteWeb }}</td>
                            <td>{{ $projet->description }}</td>
                            <td>
                                <div class="actions-icons">
                                    <a href="{{ route('admin.projets.show', $projet->id) }}">
                    ...
                    @else
                    <tr>
                        <td colspan="6">Aucun projets pour l'instant</td>
                    </tr>
                    @endif
                <tbody>
                <tfoot>
                    <tr>
                        <td colspan="6">Ici vous avez la liste de projets soumis par de jeunes entrepreneurs à la recherche d'investisseurs</td>
                    </tr>
                </tfoot>
            </table>
        </div>
